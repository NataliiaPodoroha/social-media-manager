...
def create_db_comment(
    db: Session, comment: CommentCreate, user_id: int, is_blocked: bool
) -> Comment:
    db_comment = Comment(
...
def handle_ai_reply(
    db: Session, comment: CommentCreate, user_id: int, parent_comment_id: int
) -> None:
    ...
    if user and user.auto_reply:
        ...
        ai_reply = generate_ai_reply(
            post_content, comment.content, user.reply_delay
        )
...
def create_comment(
        db: Session, comment: CommentCreate, user_id: int
) -> Comment:
    is_approved = moderate_comment(comment.content)

    db_comment = create_db_comment(
        db, comment, user_id, is_blocked=not is_approved
    )
...
def get_comment(db: Session, comment_id: int) -> Comment:
    ...
    if not db_comment or db_comment.is_blocked:
        raise HTTPException(
            status_code=404, detail="Comment not found or blocked."
        )
    return db_comment
