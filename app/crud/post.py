...
def create_post(db: Session, post: PostCreate, user_id: int) -> Post:
    is_blocked = not (moderate_text(post.content) and moderate_text(post.title))
...
def get_single_post(db: Session, post_id: int) -> Post:
    post = db.query(Post).filter(Post.id == post_id, Post.is_blocked == False).first()
    if not post:
        raise HTTPException(
            status_code=403, detail="This post has been blocked or does not exist."
        )
    return post


def get_posts_by_owner(db: Session, user_id: int, skip: int = 0, limit: int = 10) -> list[Post]:
    return (
        db.query(Post)
